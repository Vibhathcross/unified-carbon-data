...
const carbonFacts = [
  "A single mature tree can absorb up to 22 kg of carbon dioxide from the atmosphere per year.",
  "If global food waste were a country, it would be the third largest emitter of greenhouse gases after China and the USA.",
  "Switching one regular light bulb to an LED bulb reduces carbon emissions by about 50 kg per year.",
  "Public transit reduces CO2 emissions by 45% compared to driving a private vehicle alone.",
  "Eating a plant-based meal just once a week saves about 100 kg of CO2 equivalent per year.",
  "A round-trip transatlantic flight produces more carbon emissions than an average person in many developing nations does in a whole year.",
  "The ocean absorbs about 30% of all carbon dioxide emissions generated by human activity, acting as a massive buffer.",
  "Using a clothesline to dry laundry instead of a machine dryer saves about 2 kg of CO2 per load.",
  "By turning down your thermostat by just 1°C, you can reduce your home energy bill and carbon footprint by up to 10%."
]
...
export default function Dashboard({ 
  ...
  // Rotate loading facts while submitting logs
  useEffect(() => {
    if (!submitting) return
    setCurrentFactIndex(Math.floor(Math.random() * carbonFacts.length))
...
alter table journal_logs alter column category drop not null;`}
  ...
  const handleLogSubmit = async (e) => {
    ...
    if (!hasKey) {
      setSubmitError('LLM is not available: No API Key configured. Please configure your API Key in Settings to submit logs.')
      setSubmitting(false)
      setCurrentStepIndex(-1)
      return
    }
    ...
    try {
      ...
      setSubmitError(err.message || 'LLM is not available. Please verify your connection or API configuration.')
      setSubmitting(false)
      setCurrentStepIndex(-1)
      return
    }
            ...
            <AnimatePresence>
              {submitting && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, height: 'auto' }}
                  exit={{ opacity: 0, height: 0 }}
                  className="mt-4 py-8 flex flex-col items-center justify-center gap-4 overflow-hidden"
                >
